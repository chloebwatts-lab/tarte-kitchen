...
export default async function CouncilAllergenMatrixPage({
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6">
      ...
      {rows.length === 0 ? (
        <div className="rounded-lg border border-dashed border-input bg-muted/50 px-4 py-8 text-center text-sm text-muted-foreground">
        ...
        <div className="overflow-x-auto rounded-xl border border-border bg-card shadow-sm">
          <table className="w-full min-w-[900px] text-xs">
            ...
            {Array.from(byCategory.entries()).map(([category, list]) => (
              <tbody key={category} className="divide-y divide-border">
                <tr>
                  <td
                    colSpan={ALLERGEN_COLUMNS.length + 1}
                    className="bg-sage-soft/60 px-3 py-1.5 text-[11px] font-semibold uppercase tracking-wide text-sage-deep"
                  >
