...
function KitchenHome({
  ...
  templates: Awaited<ReturnType<typeof listChecklistTemplates>>
  ...
  const running = templates.filter((t) => t.todayRun)
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-3 md:grid-cols-2">
        {[...running, ...notStarted].map((t) => {
          const completion = t.todayRun
            ? t.todayRun.totalItems > 0
              ? Math.round(
                  (t.todayRun.completedItems / t.todayRun.totalItems) * 100
                )
              : 0
            : 0
