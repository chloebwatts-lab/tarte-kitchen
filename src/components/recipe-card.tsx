    ...
    <div className="min-h-screen bg-white text-foreground">
      ...
      <div className="fixed right-4 top-4 z-10 flex gap-2 print:hidden">
        <button
          ...
          className="inline-flex items-center gap-1.5 rounded-md border border-border bg-white px-3 py-1.5 text-xs font-medium text-foreground shadow-sm hover:bg-muted/50"
        >
      ...
      <div className="mx-auto max-w-[720px] px-10 py-12 print:p-0">
        ...
        {/* Allergens */}
        {allergens.length > 0 && (
          <div className="mt-6 rounded-lg border-2 border-red-text/20 bg-red-light p-4 print:border print:bg-white">
            <div className="font-serif text-[10px] font-semibold uppercase tracking-[0.14em] text-red-text">
              Contains allergens
            </div>
            <div className="mt-1.5 flex flex-wrap gap-1.5">
              {allergens.map((a) => (
                <span
                  key={a}
                  className="inline-flex items-center rounded-full border border-red-text/30 bg-white px-2.5 py-0.5 text-[11px] font-medium text-red-text"
                >
                  {ALLERGEN_LABEL[a] ?? a}
                </span>
              ))}
        ...
        {/* Components */}
        <div className="mt-8">
          <h2 className="border-b border-input pb-1 font-serif text-[10px] font-semibold uppercase tracking-[0.14em] text-muted-foreground">
            Ingredients
          </h2>
              ...
              {components.map((c, i) => (
                <tr key={i} className="border-b border-border last:border-0">
                  ...
                  <td className="py-2">
                    {c.name}
                    {c.isPrep && (
                      <span className="ml-1 inline-flex items-center rounded-sm bg-muted px-1 py-0.5 text-[9px] font-semibold uppercase tracking-wide text-muted-foreground">
                        prep
                      </span>
                    )}
                  </td>
